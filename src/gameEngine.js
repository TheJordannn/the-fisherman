...
function initializeCreatures() {
    state.fishList = [];
    state.assignedFishLanes = Array.from({ length: 15 }, (_, i) => i);
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                if (state.sessionCaughtLog.length === 0) {
                    summaryList.innerHTML = `<div class="text-[10px] uppercase text-white/30 text-center py-4">No animals caught during this voyage.</div>`;
                } else {
                    state.sessionCaughtLog.forEach(c => {
                        ...
                        div.innerHTML = `
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 flex items-center justify-center rounded-full bg-black/40" style="color: ${c.color}">
                                    ${correctSVG}
                                </div>
                                <span class="text-[10px] font-bold uppercase text-white truncate max-w-[120px]">${c.name}</span>
                            </div>
                            <span class="text-[8px] uppercase tracking-wider text-white/50">${c.rarity} &middot; ${Math.floor(c.size)}cm</span>
                        `;
